...
const BlogPostCard: React.FC<Props> = ({ title, slug, picture, description, date }) => {
  return (
    <Link href={`/blog/${slug}`} className="md:w-[calc(50%-.5rem)] mt-4 p-6 md:p-7 bg-glass hover:bg-glass-hover flex flex-col items-center">
      <img src={picture.url} alt={picture.title} className="object-cover w-full h-auto rounded-2xl"/>
      <div className="mt-4 w-full">
        <p className="text-sm">{format(new Date(date), 'PP', {locale: enUS})}</p>
